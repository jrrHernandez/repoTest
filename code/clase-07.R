...
## **[1.] Operador pipe (%>%)** sirve para simplicar código


### Veamos un ejemplo:
df = as_tibble(x = women)

df = mutate(.data = df , height_cm = height*2.54,  #genera nuevas variables
...
df <- as_tibble(df)
df <- select(df, -cod_ase_) ## seleccionar variables
df <- mutate(df,ifelse(is.na(estrato),1,estrato))


## **[2.] Combinar conjuntos de datos (adicionar filas/columnas)**
df2 = import("https://www.datos.gov.co/resource/epsv-yhtj.csv") %>% 
  as_tibble()%>% 
  select( -cod_ase_)%>% 
...
## Inspeccionar los datos:
df <- bin_cols(db_1,db_2) #adiciona columnas
...
#### **Ejemplo: right_join()**
df_r <- right_join(x=data_1, y= data_2, by=c("Casa","Visita"))

#### **Ejemplo: inner_join()**


#### **Ejemplo: full_join()**
df_f <- full_join(x=data_1, y= data_2, by= c("Casa","Visita"))
